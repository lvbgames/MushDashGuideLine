...
- Home Hero는 MushHero·Mush Dash를 같은 최상단 영역의 수동 game tab으로 전환한다. 각 게임은 로컬 공식 이미지 2장을 겹치지 않는 flex track Carousel로 제공하고 한 번에 한 장만 표시한다. 첫 MushHero 이미지만 eager/high priority이며 다른 세 Hero 이미지는 실제 선택 시 로드한다. desktop은 `minmax(0, .9fr) minmax(0, 1.1fr)`의 분리된 text/media 2열, mobile은 selector → text → CTA → media → controls 1열을 사용한다. 게임 상세와 Press 스크린샷은 공통 native scroll-snap·`dialog` lightbox를 사용한다.
