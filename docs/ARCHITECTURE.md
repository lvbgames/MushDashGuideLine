...
- site는 Home·Our Games·About·Contact·MushHero·Mush Dash의 4개 locale 명시적 static route와 sitemap을 생성한다. React/Vue/Svelte, SSR, adapter, Functions, Edge, DB, CMS는 사용하지 않는다.
...
- 신규 구현은 legacy 코드를 복사하거나 import하지 않는다. 유일한 예외는 보호된 byte-identical `site/public/privacy.html`이다.
