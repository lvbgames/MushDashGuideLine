...
- `analytics/`: Cloudflare Workers Free와 D1 Free용 독립 TypeScript Worker이며 Astro·Netlify build에 포함하지 않는다. Worker·D1·매일 00:10 KST Cron과 사이트 endpoint는 production active다. `daily_visitors`에는 KST 날짜, 당일 HMAC-SHA256 hash, 생성 시각만 임시 저장하고, Cron이 과거 날짜의 수를 `daily_stats`에 확정한 뒤 같은 D1 batch에서 해당 hash를 삭제한다. 장기 데이터는 날짜·최종 고유 방문자 수·집계 완료 시각뿐이며 원본 IP·User-Agent·URL·referrer·국가·세션은 저장하지 않는다. 로컬 hardening source는 HTTPS 선행 검사, HSTS, D1 이전의 Cloudflare Workers Rate Limiting binding을 추가하며 운영 반영은 별도 승인 대상이다.
