...
| `legacy-site/public/privacy.html` | 역사 보관 | SHA-256 기준 원본을 수정·이동·삭제하지 않는다. 이전 `site/public/privacy.html` 복제본은 2026-07-31 사용자 승인으로 폐기했고, 현재 정책에는 재사용하지 않는다. |
...
| 단계 | 범위 | 선행 조건 | 위험 | 검증 |
|---|---|---|---|---|
| 1 | Privacy 역사 보관·배포 기준 고정 | Netlify UI 확인 | locale route·redirect 회귀 | legacy 원본 해시와 네 Astro route 비교 |
| 2 | 디자인 토큰·전역 스타일 | 승인 브랜드 자산 | 기존 route 영향 | 기존/신규 route 시각 점검 |
| 3 | English 기본 i18n 데이터 구조 | locale 정책 확정 | locale route 불일치 | locale fallback·Privacy section 순서 |
