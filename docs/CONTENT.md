...
| MushDash 미디어 | Verified Media | Home은 기존 검증 Steam source를 유지한다. Press는 `references/LvbResult`의 키아트·와이드·800×450 프로모션 이미지 3개를 사용하며 프로모션 이미지를 플레이 스크린샷으로 표기하지 않음 |
...
| 공식 YouTube | Verified External Link / User Review | Lv.B 공식 Shorts `at6bQPAzLkI`·`GMaEwI8qMpA`. MushHero 상세에서 로컬 poster를 먼저 표시하고 Play 뒤 선택 영상만 `youtube-nocookie.com` iframe으로 로드 |
| News & Press | Verified External Link / Draft / User Review | 외부 12건·자체 글 3건, 총 15건을 최신순 6/6/3으로 제공한다. BGC 인터뷰, MushHero VFX 개발일지, MushDash 앞서 해보기 공지를 추가했으며 MushHero X 공지는 개별 URL·날짜 미확인으로 보류한다. |
| Press Kit | Draft / User Review | `references/LvbResult` 승인 원본에서 브랜드 5개·MushHero 7개·MushDash 6개를 유지하며, 기존 공개 URL의 ZIP 3개에는 Brand README/Guide와 두 게임의 EN·KO·JA·ZH-CN Fact Sheet/README를 제공한다. 화면은 대표 키아트·공식 게임 이미지와 locale boilerplate 복사를 유지하고 Recent Press는 표시하지 않는다. Header/Footer CTA는 현재 locale의 Press route를 사용한다. source·SHA·ZIP 내용과 갱신 절차는 `PRESS_KIT.md`를 따른다. |
| Privacy 4개 언어 | Draft / User Review | 기존 19개 section과 게임·이메일 처리 사실을 유지하고 click-to-load YouTube privacy-enhanced embed를 추가했다. 최종 수정일·시행일은 `2026-08-26`; `/privacy/` 및 세 locale route는 `noindex, follow`이고 sitemap에서 제외 |
...
| 검색·공유 메타데이터 | Verified Public Metadata / User Review | `site/src/data/socialImages.ts`가 Lv.B·MushHero·MushDash 전용 1200×630 이미지와 locale별 alt를 관리한다. `BaseLayout.astro`는 canonical·hreflang과 함께 절대 OG/Twitter image URL, width·height·MIME·alt를 출력하며 Privacy·Terms의 기존 noindex 정책은 유지한다. |

공식명·홈페이지·이메일·SNS·게임명·Store URL·플랫폼·출시 상태 등 안정적인 사실은 `site/src/data/siteFacts.json`에서 관리하고 `games.ts`·`contact.ts`·`socialLinks.ts`가 필요한 shape로 가공한다. 브라우저·빌드 중 runtime scraping은 하지 않는다. 기존 사이트의 코드·문구·자산은 `legacy-site/` 보관물일 뿐 신규 구현의 기반이 아니다.

Privacy의 처리 사실 근거와 운영자 확인 항목은 `PRIVACY_DATA_INVENTORY.md`에서, MushDash UserCloud의 내부 파일·field·호출 라인은 `PRIVACY_USERCLOUD_AUDIT.md`에서 관리한다. 공개 문구는 검증한 데이터 범주만 사용하고 내부 key·변수명·development 진단 payload는 노출하지 않는다. UserCloud의 고정 보유기간과 게임 내 삭제 기능은 없으며, 요청 접수·본인/범위 확인·플랫폼/EOS 확인은 내부 전용 `PRIVACY_REQUEST_RUNBOOK.md`를 따른다. 외부 사업자의 위탁·제3자 제공·독립 처리 관계와 국외 이전 유형도 법률 확인 전 단정하지 않는다. YouTube 고지 추가에 따라 공개 Privacy의 최종 수정일과 시행일은 `2026-08-26`이다.
